...
import com.zoutiw.bibla.data.BibleRepository
import com.zoutiw.bibla.data.UserProgress
...
class BibleViewModel(application: Application) : AndroidViewModel(application) {
    ...
    private val _comments = MutableStateFlow<List<com.zoutiw.bibla.data.VerseComment>>(emptyList())
    val comments: StateFlow<List<com.zoutiw.bibla.data.VerseComment>> = _comments.asStateFlow()
    ...
    private fun resolveCanonicalBook(rawInput: String): String? {
        val canonicalFromName = com.zoutiw.bibla.ui.util.BibleBookNames.toCanonical(rawInput)
        if (com.zoutiw.bibla.data.BibleData.allBooks.contains(canonicalFromName)) return canonicalFromName
        ...
        flow {
            ...
            val filtered = combined.filter { verse ->
                ...
                val matchesTestament = when (testamentFilter) {
                    ...
                    SearchTestamentFilter.OLD_TESTAMENT -> verse.book in com.zoutiw.bibla.data.BibleData.oldTestament
                    SearchTestamentFilter.NEW_TESTAMENT -> verse.book in com.zoutiw.bibla.data.BibleData.newTestament
                }
    ...
    private val frenchRepository = com.zoutiw.bibla.data.FrenchBibleRepository(application)

    private val _appLanguage = MutableStateFlow(
        prefs.getString("pref_app_language", com.zoutiw.bibla.ui.util.AppLanguage.HT) ?: com.zoutiw.bibla.ui.util.AppLanguage.HT
    )
    val appLanguage: StateFlow<String> = _appLanguage

    private val _bibleVersion = MutableStateFlow(
        run {
            val savedLang = prefs.getString("pref_app_language", com.zoutiw.bibla.ui.util.AppLanguage.HT) ?: com.zoutiw.bibla.ui.util.AppLanguage.HT
            val savedVersion = prefs.getString("pref_bible_version", null)
            when {
                savedVersion == com.zoutiw.bibla.ui.util.BibleVersion.BILINGUAL -> com.zoutiw.bibla.ui.util.BibleVersion.BILINGUAL
                savedLang == com.zoutiw.bibla.ui.util.AppLanguage.FR -> com.zoutiw.bibla.ui.util.BibleVersion.FRANCAIS_LSG
                savedVersion != null -> savedVersion
                else -> com.zoutiw.bibla.ui.util.BibleVersion.KREYOL
            }
        }
    )
    ...
    fun setAppLanguage(lang: String) {
        ...
        val targetVersion = if (lang == com.zoutiw.bibla.ui.util.AppLanguage.FR) {
            com.zoutiw.bibla.ui.util.BibleVersion.FRANCAIS_LSG
        } else {
            com.zoutiw.bibla.ui.util.BibleVersion.KREYOL
        }
    ...
    fun loadFrenchVersesIfNeeded(book: String, chapter: Int, version: String = _bibleVersion.value) {
        if (version == com.zoutiw.bibla.ui.util.BibleVersion.FRANCAIS_LSG || version == com.zoutiw.bibla.ui.util.BibleVersion.BILINGUAL) {
    ...
    private fun loadAllCompletedPlanDays(): Map<String, Set<Int>> {
        ...
        for (plan in com.zoutiw.bibla.data.ReadingPlanRepository.allPlans) {
    ...
    private fun scheduleReadingPlanReminderWork() {
        ...
        val workRequest = androidx.work.PeriodicWorkRequestBuilder<com.zoutiw.bibla.notifications.DailyReminderWorker>(
    ...
    fun setNotificationsEnabled(enabled: Boolean) {
        ...
        val context = getApplication<Application>().applicationContext
        if (enabled) {
            val workRequest = androidx.work.PeriodicWorkRequestBuilder<com.zoutiw.bibla.notifications.DailyReminderWorker>(24, java.util.concurrent.TimeUnit.HOURS)
                .build()
            androidx.work.WorkManager.getInstance(context).enqueueUniquePeriodicWork(
                "daily_reminder",
    ...
    private fun triggerSystemNotification(context: android.content.Context, title: String, message: String) {
        try {
            ...
            if (notificationManager != null) {
                ...
                val intent = android.content.Intent(context, com.zoutiw.bibla.MainActivity::class.java).apply {
                ...
                val builder = androidx.core.app.NotificationCompat.Builder(context, channelId)
                    .setSmallIcon(com.zoutiw.bibla.R.drawable.ic_notification_bib_la)
    ...
    private fun getInitialSeedComments(): List<com.zoutiw.bibla.data.VerseComment> {
        val now = System.currentTimeMillis()
        return listOf(
            com.zoutiw.bibla.data.VerseComment(
                id = "seed_comm_1",
                verseId = 1L,
    ...
    private fun parseCommentsFromFirebase(jsonStr: String): List<com.zoutiw.bibla.data.VerseComment> {
        ...
        if (trimmed.startsWith("[")) {
            ...
            val obj = org.json.JSONObject(trimmed)
            val list = mutableListOf<com.zoutiw.bibla.data.VerseComment>()
            val keys = obj.keys()
            while (keys.hasNext()) {
                val key = keys.next()
    ...
    private fun fetchCommentsFromFirebase(client: okhttp3.OkHttpClient): List<com.zoutiw.bibla.data.VerseComment> {
        val firebaseUrls = listOf(
    ...
    private fun pushCommentToFirebase(client: okhttp3.OkHttpClient, comment: com.zoutiw.bibla.data.VerseComment) {
        try {
    ...
    private fun mergeAndSaveComments(cloudList: List<com.zoutiw.bibla.data.VerseComment>) {
        if (cloudList.isEmpty()) return
        val currentLocal = _comments.value
        val mergedMap = mutableMapOf<String, com.zoutiw.bibla.data.VerseComment>()
    ...
    private fun parseCommentsJson(arr: org.json.JSONArray?): List<com.zoutiw.bibla.data.VerseComment> {
    ...
    private fun commentsToJson(list: List<com.zoutiw.bibla.data.VerseComment>): org.json.JSONArray {
    ...
    fun addComment(text: String, parentId: String? = null, userName: String) {
        ...
        val newComment = com.zoutiw.bibla.data.VerseComment(
        ...
        viewModelScope.launch(kotlinx.coroutines.Dispatchers.IO) {
            try {
                ...
                val mergeMap = mutableMapOf<String, com.zoutiw.bibla.data.VerseComment>()
    ...
    private fun syncAllToCloud(updatedComments: List<com.zoutiw.bibla.data.VerseComment>? = null) {
    ...
    fun setVerses(book: String, chapter: Int) {
        val canonical = com.zoutiw.bibla.ui.util.BibleBookNames.toCanonical(book)
    ...
    fun deleteNote(note: com.zoutiw.bibla.data.Note) {
        viewModelScope.launch {
            repository.deleteNote(note)
        }
    }

    suspend fun getNoteById(id: Long): com.zoutiw.bibla.data.Note? {
        return repository.getNoteById(id)
    }

    fun updateNote(note: com.zoutiw.bibla.data.Note) {
        viewModelScope.launch {
            repository.updateNote(note)
        }
    }

    fun toggleBookmark(verse: com.zoutiw.bibla.data.Verse, color: Int = 0) {
        viewModelScope.launch {
            repository.toggleBookmark(verse, color)
        }
    }

    suspend fun getVerseById(id: Long): com.zoutiw.bibla.data.Verse? {
